...
select concat('CFW','||',Policy_Number,'||',Effective_Date) Product_specification_key,  nvl(concat('CFW','||',Policy_Number,'||',
              Effective_Date,'||',building_number,'||',PRMSTE,'||', CLASX,'||', SUBLN,'||',internal_coverage),'NA') Coverage_Key , Limit_Type, Limit_Option, Limit_Amount,
...
select Product_specification_key, Coverage_Key ,Limit_Type, Limit_Option, Limit_Amount,
Effective_Date, Expiration_Date , Start_Date, sha2(concat(Product_specification_key,'||', Coverage_Key ,'||',Limit_Type,'||', Limit_Option,'||', Limit_Amount,'||',
