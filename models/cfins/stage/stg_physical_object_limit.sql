...
with physical_object_limit_fc as
(
...
ltrim(t2.INTCOV,0) internal_coverage,date(nullifzero(t1.EFFDTE),'YYYYMMDD') Effective_Date,
...
select concat('CFW','||',Policy_Number,'||',Effective_Date) Product_specification_key,  nvl(concat('CFW','||',Policy_Number,'||',
              Effective_Date,'||',building_number,'||',PRMSTE,'||', CLASX,'||', SUBLN,'||',internal_coverage),'NA') Coverage_Key ,nvl(concat(Location_number,'||',Building_number,'||',Policy_Number),'NA') as Physical_object_Key, Limit_Type, Limit_Option, Limit_Amount,
Effective_Date, Expiration_Date , dateadd(ns,EDSNO*100,Effective_Date) as Start_Date
from physical_object_limit_fc
UNION ALL
select concat('FCW','||',Policy_Number,'||',Effective_Date) Product_specification_key,  nvl(concat('CFW','||',Policy_Number,'||',
              Effective_Date,'||',building_number,'||',PRMSTE,'||', CLASX,'||', SUBLN,'||',internal_coverage),'NA') Coverage_Key, nvl(concat(Location_number,'||',Building_number,'||',Policy_Number),'NA') as Physical_object_Key,Limit_Type, Limit_Option, Limit_Amount,
Effective_Date, Expiration_Date , dateadd(ns,EDSNO*100,Effective_Date) as Start_Date
from physical_object_limit_cf)

select Product_specification_key, Coverage_Key ,Physical_object_Key,Limit_Type, Limit_Option, Limit_Amount,
Effective_Date, Expiration_Date , Start_Date, sha2(concat(Product_specification_key,'||', Coverage_Key ,'||',Physical_object_Key,'||',Limit_Type,'||', Limit_Option,'||', Limit_Amount,'||',
