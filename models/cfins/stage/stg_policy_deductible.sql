...
select distinct ltrim(t1.POLICY,0) Policy_Number,date(nullifzero(t1.EFFDTE),'YYYYMMDD') Effective_Date,
   date(nullifzero(t2.EXPDTE),'YYYYMMDD') Expiration_Date , t1.EDSNO, GPRDDT as Deductible_Type, 
...
UNION ALL
select distinct ltrim(t1.POLICY,0) Policy_Number,date(nullifzero(t1.EFFDTE),'YYYYMMDD') Effective_Date,
   date(nullifzero(t2.EXPDTE),'YYYYMMDD') Expiration_Date , t1.EDSNO, GPMDDT as Deductible_Type, 
